...
genus_m6  <- genus[rownames(genus) %in% weeks[['6']]$JAC.Mouse.ID, ]
genus_m6  <- genus_m6[, colSums(genus_m6 > 0) > (nrow(genus_m6) * 0.05)]


genus_m12 <- genus[rownames(genus) %in% weeks[['12']]$JAC.Mouse.ID, ]
genus_m12 <- genus_m12[, colSums(genus_m12 > 0) > (nrow(genus_m12) * 0.05)]


genus_m18 <- genus[rownames(genus) %in% weeks[['18']]$JAC.Mouse.ID, ]
genus_m18 <- genus_m18[, colSums(genus_m18 > 0) > (nrow(genus_m18) * 0.05)]
...
### Normalize genus using Hoans'/Dong-bing's normalization. 
...
norm_m6  <- apply(genus_m6, 2, function(x) x / sum(x))
norm_m6  <- norm_m6 + 1
norm_m6  <- codaSeq.clr(x = norm_m6, samples.by.row = TRUE)

norm_m12  <- apply(genus_m12, 2, function(x) x / sum(x))
norm_m12  <- norm_m12 + 1
norm_m12 <- t(transform(t(norm_m12), transform = 'clr', target = 'sample'))

norm_m18  <- apply(genus_m18, 2, function(x) x / sum(x))
norm_m18  <- norm_m18 + 1
...
taxa_m6  <- taxa %>% filter(Genus %in% colnames(genus_m6))
taxa_m12 <- taxa %>% filter(Genus %in% colnames(genus_m12))
taxa_m18 <- taxa %>% filter(Genus %in% colnames(genus_m18))
...
samples <- samples %>%
              ...
              filter(mouse.id %in% c(rownames(genus_m6), rownames(genus_m12), rownames(genus_m18))) %>%
              distinct()

samples_m6  <- samples %>% filter(mouse.id %in% rownames(genus_m6))
samples_m12 <- samples %>% filter(mouse.id %in% rownames(genus_m12))
samples_m18 <- samples %>% filter(mouse.id %in% rownames(genus_m18))
...
### Annotate columns
annot.phenotype <- data.frame(data.name   = c(colnames(samples), taxa[,'Genus']),
                              short.name  = c(colnames(samples), taxa[,'Genus']),
                              R.name      = c(colnames(samples), taxa[,'Genus']),
...
annot.pheno.m6  <- annot.phenotype %>% filter(data.name %in% c(colnames(samples), colnames(genus_m6)))
annot.pheno.m12 <- annot.phenotype %>% filter(data.name %in% c(colnames(samples), colnames(genus_m12)))
annot.pheno.m18 <- annot.phenotype %>% filter(data.name %in% c(colnames(samples), colnames(genus_m18)))
...
### QTL viewer format
dataset.genus.m6 <- list(annot.phenotype = as_tibble(annot.pheno.m6),
                         ...
                         data            = list(norm = as.matrix(norm_m6),
                                                raw  = as.matrix(genus_m6)),
                         datatype        = 'phenotype',
                         display.name    = 'DOMA Genus Abundance Month 6',
                         lod.peaks       = list(),
                         taxa            = as_tibble(taxa_m6))


dataset.genus.m12 <- list(annot.phenotype = as_tibble(annot.pheno.m12),
                          annot.samples   = as_tibble(samples_m12),
                          covar.matrix    = as.matrix(covar_m12),
                          covar.info      = as_tibble(covar.info),
                          data            = list(norm = as.matrix(norm_m12),
                                                 raw  = as.matrix(genus_m12)),
                          datatype        = 'phenotype',
                          display.name    = 'DOMA Genus Abundance Month 12',
                          lod.peaks       = list(),
                          taxa            = as_tibble(taxa_m12))


dataset.genus.m18 <- list(annot.phenotype = as_tibble(annot.pheno.m18),
                          annot.samples   = as_tibble(samples_m18),
                          covar.matrix    = as.matrix(covar_m18),
                          covar.info      = as_tibble(covar.info),
                          data            = list(norm = as.matrix(norm_m18),
                                                 raw  = as.matrix(genus_m18)),
                          datatype        = 'phenotype',
                          display.name    = 'DOMA Genus Abundance Month 18',
                          lod.peaks       = list(),
