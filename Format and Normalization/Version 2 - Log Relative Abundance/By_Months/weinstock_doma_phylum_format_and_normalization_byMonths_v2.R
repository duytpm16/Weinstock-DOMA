...
load('~/Desktop/Weinstock_DOMA/Phenotypes/doma_otu_16s_data/Modified/0.5/phylum_raw_count_and_taxa_0.5.Rdata')
...
phylum_m6 <- phylum[rownames(phylum) %in% weeks[['6']]$JAC.Mouse.ID, ]
phylum_m6 <- phylum_m6[, colSums(phylum_m6 > 0) > (nrow(phylum_m6) * 0.05)]


phylum_m12 <- phylum[rownames(phylum) %in% weeks[['12']]$JAC.Mouse.ID, ]
phylum_m12 <- phylum_m12[, colSums(phylum_m12 > 0) > (nrow(phylum_m12) * 0.05)]


phylum_m18 <- phylum[rownames(phylum) %in% weeks[['18']]$JAC.Mouse.ID, ]
phylum_m18 <- phylum_m18[, colSums(phylum_m18 > 0) > (nrow(phylum_m18) * 0.05)]
...
### Normalize phylum using Hoans'/Dong-bing's normalization. 
...
norm_m6  <- apply(phylum_m6, 2, function(x) x / sum(x))
norm_m6  <- norm_m6 + 1
norm_m6  <- codaSeq.clr(x = norm_m6, samples.by.row = TRUE)

norm_m12 <- apply(phylum_m12, 2, function(x) x / sum(x))
norm_m12 <- norm_m12 + 1
norm_m12 <- codaSeq.clr(x = norm_m12, samples.by.row = TRUE)

norm_m18 <- apply(phylum_m18, 2, function(x) x / sum(x))
norm_m18 <- norm_m18 + 1
...
taxa_m6  <- taxa %>% filter(Phylum %in% colnames(phylum_m6))
taxa_m12 <- taxa %>% filter(Phylum %in% colnames(phylum_m12))
taxa_m18 <- taxa %>% filter(Phylum %in% colnames(phylum_m18))
...
samples <- samples %>%
              ...
              filter(mouse.id %in% c(rownames(phylum_m6), rownames(phylum_m12), rownames(phylum_m18))) %>%
              distinct()

samples_m6  <- samples %>% filter(mouse.id %in% rownames(phylum_m6))
samples_m12 <- samples %>% filter(mouse.id %in% rownames(phylum_m12))
samples_m18 <- samples %>% filter(mouse.id %in% rownames(phylum_m18))
...
covar_m6  <- covar[rownames(covar) %in% rownames(phylum_m6),]
covar_m12 <- covar[rownames(covar) %in% rownames(phylum_m12),]
covar_m18 <- covar[rownames(covar) %in% rownames(phylum_m18),]
...
### Annotate columns
annot.phenotype <- data.frame(data.name   = c(colnames(samples), taxa[,'Phylum']),
                              short.name  = c(colnames(samples), taxa[,'Phylum']),
                              R.name      = c(colnames(samples), taxa[,'Phylum']),
...
annot.pheno.m6  <- annot.phenotype %>% filter(data.name %in% c(colnames(samples), colnames(phylum_m6)))
annot.pheno.m12 <- annot.phenotype %>% filter(data.name %in% c(colnames(samples), colnames(phylum_m12)))
annot.pheno.m18 <- annot.phenotype %>% filter(data.name %in% c(colnames(samples), colnames(phylum_m18)))
...
### QTL viewer format
dataset.phylum.m6 <- list(annot.phenotype = as_tibble(annot.pheno.m6),
                          ...
                          data            = list(norm = as.matrix(norm_m6),
                                                 raw  = as.matrix(phylum_m6)),
                          datatype        = 'phenotype',
                          display.name    = 'DOMA Phylum Abundance Month 6',
                          lod.peaks       = list(),
                          taxa            = as_tibble(taxa_m6))


dataset.phylum.m12 <- list(annot.phenotype = as_tibble(annot.pheno.m12),
                           annot.samples   = as_tibble(samples_m12),
                           covar.matrix    = as.matrix(covar_m12),
                           covar.info      = as_tibble(covar.info),
                           data            = list(norm = as.matrix(norm_m12),
                                                  raw  = as.matrix(phylum_m12)),
                           datatype        = 'phenotype',
                           display.name    = 'DOMA Phylum Abundance Month 12',
                           lod.peaks       = list(),
                           taxa            = as_tibble(taxa_m12))


dataset.phylum.m18 <- list(annot.phenotype = as_tibble(annot.pheno.m18),
                           annot.samples   = as_tibble(samples_m18),
                           covar.matrix    = as.matrix(covar_m18),
                           covar.info      = as_tibble(covar.info),
                           data            = list(norm = as.matrix(norm_m18),
                                                  raw  = as.matrix(phylum_m18)),
                           datatype        = 'phenotype',
                           display.name    = 'DOMA Phylum Abundance Month 18',
                           lod.peaks       = list(),
