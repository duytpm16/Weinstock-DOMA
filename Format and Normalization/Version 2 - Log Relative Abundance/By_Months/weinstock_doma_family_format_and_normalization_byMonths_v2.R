...
family_m6 <- family[rownames(family) %in% weeks[['6']]$JAC.Mouse.ID, ]
family_m6 <- family_m6[, colSums(family_m6 > 0) > (nrow(family_m6) * 0.05)]


family_m12 <- family[rownames(family) %in% weeks[['12']]$JAC.Mouse.ID, ]
family_m12 <- family_m12[, colSums(family_m12 > 0) > (nrow(family_m12) * 0.05)]


family_m18 <- family[rownames(family) %in% weeks[['18']]$JAC.Mouse.ID, ]
family_m18 <- family_m18[, colSums(family_m18 > 0) > (nrow(family_m18) * 0.05)]
...
### Normalize family using Hoans'/Dong-bing's normalization. 
...
norm_m6  <- apply(family_m6, 2, function(x) x / sum(x))
norm_m6  <- norm_m6 + 1
norm_m6  <- codaSeq.clr(x = norm_m6, samples.by.row = TRUE)

norm_m12  <- apply(family_m12, 2, function(x) x / sum(x))
norm_m12  <- norm_m12 + 1
norm_m12 <- t(transform(t(norm_m12), transform = 'clr', target = 'sample'))

norm_m18  <- apply(family_m18, 2, function(x) x / sum(x))
norm_m18  <- norm_m18 + 1
...
taxa_m6  <- taxa %>% filter(Family %in% colnames(family_m6))
taxa_m12 <- taxa %>% filter(Family %in% colnames(family_m12))
taxa_m18 <- taxa %>% filter(Family %in% colnames(family_m18))
...
samples <- samples %>%
              ...
              filter(mouse.id %in% c(rownames(family_m6), rownames(family_m12), rownames(family_m18))) %>%
              distinct()

samples_m6  <- samples %>% filter(mouse.id %in% rownames(family_m6))
samples_m12 <- samples %>% filter(mouse.id %in% rownames(family_m12))
samples_m18 <- samples %>% filter(mouse.id %in% rownames(family_m18))
...
covar_m6  <- covar[rownames(covar) %in% rownames(family_m6),]
covar_m12 <- covar[rownames(covar) %in% rownames(family_m12),]
covar_m18 <- covar[rownames(covar) %in% rownames(family_m18),]
...
### Annotate columns
annot.phenotype <- data.frame(data.name   = c(colnames(samples), taxa[,'Family']),
                              short.name  = c(colnames(samples), taxa[,'Family']),
                              R.name      = c(colnames(samples), taxa[,'Family']),
...
annot.pheno.m6  <- annot.phenotype %>% filter(data.name %in% c(colnames(samples), colnames(family_m6)))
annot.pheno.m12 <- annot.phenotype %>% filter(data.name %in% c(colnames(samples), colnames(family_m12)))
annot.pheno.m18 <- annot.phenotype %>% filter(data.name %in% c(colnames(samples), colnames(family_m18)))
...
### QTL viewer format
dataset.family.m6 <- list(annot.phenotype = as_tibble(annot.pheno.m6),
                          ...
                          data            = list(norm = as.matrix(norm_m6),
                                                 raw  = as.matrix(family_m6)),
                          datatype        = 'phenotype',
                          display.name    = 'DOMA Family Abundance Month 6',
                          lod.peaks       = list(),
                          taxa            = as_tibble(taxa_m6))


dataset.family.m12 <- list(annot.phenotype = as_tibble(annot.pheno.m12),
                           annot.samples   = as_tibble(samples_m12),
                           covar.matrix    = as.matrix(covar_m12),
                           covar.info      = as_tibble(covar.info),
                           data            = list(norm = as.matrix(norm_m12),
                                                  raw  = as.matrix(family_m12)),
                           datatype        = 'phenotype',
                           display.name    = 'DOMA Family Abundance Month 12',
                           lod.peaks       = list(),
                           taxa            = as_tibble(taxa_m12))


dataset.family.m18 <- list(annot.phenotype = as_tibble(annot.pheno.m18),
                           annot.samples   = as_tibble(samples_m18),
                           covar.matrix    = as.matrix(covar_m18),
                           covar.info      = as_tibble(covar.info),
                           data            = list(norm = as.matrix(norm_m18),
                                                  raw  = as.matrix(family_m18)),
                           datatype        = 'phenotype',
                           display.name    = 'DOMA Family Abundance Month 18',
                           lod.peaks       = list(),
