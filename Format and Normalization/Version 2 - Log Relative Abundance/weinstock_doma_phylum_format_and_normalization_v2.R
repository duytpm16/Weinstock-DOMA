...
### Removing phylum with <= 5% prevalence according to Hoan
#     phylum: 403 x 8
phylum_counts <- phylum_counts[, colSums(phylum_counts > 0) > (nrow(phylum_counts) * 0.05)]
phylum_taxa   <- phylum_taxa[,ncol(phylum_taxa):1]
phylum_taxa   <- phylum_taxa %>% filter(Phylum %in% colnames(phylum_counts))
...
samples <- samples %>%
              ...
              filter(Mouse.ID %in% rownames(phylum_counts)) %>%
...
### Normalize phylum using Hoan / Dong-binh's code
...
norm <- apply(phylum_counts, 2, function(x) x / sum(x))
norm <- norm + 1
...
### Annotate columns
annot.phenotype <- data.frame(data.name   = c(colnames(samples), phylum_taxa[,'Phylum']),
                              short.name  = c(colnames(samples), phylum_taxa[,'Phylum']),
                              R.name      = c(colnames(samples), phylum_taxa[,'Phylum']),
                              description = c('Mouse identifier', 'Gender of mouse: F (female) or M (male)', 'Mouse date-of-birth', 'Generation of mouse', 'Age of mouse in months',
                                              ...
                                              'Cage of mouse', 'Ear notch', 'JAC Cohort', 'chrM', 'chrY', apply(phylum_taxa, 1, paste0, collapse = ' - ')),
                              units       = NA,
                              category    = c(rep('Demographic', ncol(samples)), rep('Microbiome', nrow(phylum_taxa))),
                              R.category  = c(rep('Demographic', ncol(samples)), rep('Microbiome', nrow(phylum_taxa))),
                              is.id       = c(TRUE, rep(FALSE, ncol(samples) + nrow(phylum_taxa) - 1)),
                              is.numeric  = c(FALSE, FALSE, FALSE, FALSE, TRUE, FALSE, FALSE, FALSE, TRUE, TRUE, TRUE, FALSE, FALSE, FALSE, FALSE, FALSE,
                                              rep(TRUE, nrow(phylum_taxa))),
                              is.date     = c(FALSE, FALSE, TRUE, FALSE, FALSE, FALSE, TRUE, TRUE, rep(FALSE, ncol(samples) + nrow(phylum_taxa) - 8)),
                              is.factor   = c(FALSE, TRUE, FALSE, TRUE, rep(FALSE, ncol(samples) + nrow(phylum_taxa) - 4)),
                              factor.levels = c(NA, 'F:M', NA, '8:9:10:11:12', rep(FALSE, ncol(samples) + nrow(phylum_taxa) - 4)),
                              is.covar    = c(FALSE, TRUE, FALSE, TRUE, TRUE, rep(FALSE, ncol(samples) + nrow(phylum_taxa) - 5)),
                              is.pheno    = c(rep(FALSE, ncol(samples)), rep(TRUE, nrow(phylum_taxa))),
                              is.derived  = FALSE,
                              omit        = FALSE,
                              use.covar   = c(rep(NA, ncol(samples)), rep('sex:cohort.age:generation', nrow(phylum_taxa))),
                              transformation = c(rep(NA, ncol(samples)), rep('rankz', nrow(phylum_taxa))))
