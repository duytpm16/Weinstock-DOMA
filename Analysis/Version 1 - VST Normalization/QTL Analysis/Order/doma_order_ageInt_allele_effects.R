...
peaks <- dataset.doma.order$lod.peaks$age_int
...
m6_order  <- order[samps$mouse.id[samps$cohort.age == 6], ]
m12_order <- order[samps$mouse.id[samps$cohort.age == 12], ]
m18_order <- order[samps$mouse.id[samps$cohort.age == 18], ]
...
for(i in 1:nrow(peaks)){
    ...
    m6_blup  <- scan1blup(genoprobs = gp,
                          pheno     = m6_order[,peaks$data.name[i], drop = FALSE],
                          kinship   = K[[peaks$qtl.chr[i]]],
                          addcovar  = covar)
  
    m12_blup <- scan1blup(genoprobs = gp,
                          pheno     = m12_order[,peaks$data.name[i], drop = FALSE],
                          kinship   = K[[peaks$qtl.chr[i]]],
                          addcovar  = covar)
    
    m18_blup <- scan1blup(genoprobs = gp,
                          pheno     = m18_order[,peaks$data.name[i], drop = FALSE],
                          kinship   = K[[peaks$qtl.chr[i]]],
                          addcovar  = covar)
...
dataset.doma.order$lod.peaks$age_int <- peaks
