...
### Estimate heritability per chromosome for each order
chr_herit <- as.data.frame(matrix(0, nrow = 20, ncol = ncol(order),
                                  dimnames = list(paste0('chr',c(1:19, 'X')), colnames(order))))

for(i in colnames(chr_herit)){
    for(j in rownames(chr_herit)){
        chr = gsub('chr', '', j)
        chr_herit[j, i] <- est_herit(pheno    = order[, i, drop = FALSE], 
                                     kinship  = K[[chr]],
                                     addcovar = covar,
                                     cores    = 0)[1]
    }
...
### Estimate overall heritability for each order
K_overall <- calc_kinship(probs = genoprobs, type = 'overall', cores = 0)

overall_herit <- as.data.frame(matrix(0, nrow = 1, ncol = ncol(order),
                                      dimnames = list('overall', colnames(order))))

for(i in colnames(overall_herit)){
    overall_herit[1, i] <- est_herit(pheno    = order[, i, drop = FALSE], 
                                     kinship  = K_overall,
                                     addcovar = covar,
                                     cores    = 0)[1]
    print(i)
...
### Save
dataset.doma.order$herit <- list(overall = overall_herit, 
